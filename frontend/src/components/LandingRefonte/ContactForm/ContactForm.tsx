...
import tardis from "../../../assets/images/tardis.webp";
import { useAddDoc } from "../../../hooks/use-firebase";
import { useScores } from "../../../contexts/ScoreContext";
import { useSendContactBrevo } from "../../../hooks/use-brevo";
...
export default function ContactForm() {
  ...
  const listId = parseInt(import.meta.env.VITE_BREVO_LIST_ID, 10); // Get the list ID from environment variables
  const { addDocument } = useAddDoc(
    import.meta.env.VITE_FIREBASE_COLLECTION_SUBMISSIONS
  );
  const { sendContact } = useSendContactBrevo([listId]);
